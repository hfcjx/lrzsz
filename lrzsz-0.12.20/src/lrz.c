...
reportdetail(struct zm_fileinfo *zi)
{
	if (Verbose>1){
		...
		if (d==0)
			d=0.5; /* can happen if timing uses time() */
        d_bps = (double)(zi->bytes_received-zi->bytes_skipped) / d;
		bps = d_bps;
        if (bps > 0) {
            d_bps = R_BYTESLEFT(zi) / d_bps;
		...
		if (Verbose > 1) {
            if (protocol==ZM_XMODEM) {
                vstringf(_("\rBytes received: %7ld/ %7ld   BPS:%-6ld ETA %02d:%02d.%03d  \n"),
						(long) zi->bytes_received, (long) -1, 
						bps, -1, -1,-1);
            }else{
                vstringf(_("\rBytes received: %7ld/ %7ld   BPS:%-6ld ETA %02d:%02d.%03d  \n"),
