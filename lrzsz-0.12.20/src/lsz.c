...
//hfcjx
//xymodem keep consistent with zmodem
static void
reportdetail(struct zm_fileinfo *zi)
{
	if (Verbose>1){
		double d;
		long bps;
		...
		int secleft = 0;
        int mssecleft = 0;
		size_t send_len = zi->bytes_sent;
		d=timing(0,NULL);
		if (d==0)
			d=0.5; /* can happen if timing uses time() */
        d_bps = (double)zi->bytes_sent / d;
		bps = d_bps;
        if (bps > 0) {
            if(zi->bytes_sent > zi->bytes_total) send_len = zi->bytes_total;
            d_bps = (zi->bytes_total - send_len) / d_bps;
			minleft = (int)d_bps / 60;
			secleft = (int)d_bps % 60;
            d_bps = d_bps - (int)d_bps;
            mssecleft = d_bps * 1000;
		}
		if (Verbose > 1) {
			vchar ('\r');
			vstringf (_("Bytes Sent: %7ld/ %7ld   BPS:%-8ld ETA %02d:%02d.%03d  \n"),
				 (long) send_len, (long) zi->bytes_total, 
				bps, minleft, secleft,mssecleft);
		}
	}
}
  ...
  somemore:
	...
	do {
		...
		if ((Verbose > 1 || min_bps || stop_time)
			...
			d_bps = (zi->bytes_sent / timing (0,&now));
            last_bps  = d_bps;
            //
			if (last_bps > 0) {
                //hfcjx
				...
				d_bps = (zi->bytes_total - zi->bytes_sent) / d_bps;
    			minleft = (int)d_bps / 60;
    			secleft = (int)d_bps % 60;
                d_bps = d_bps - (int)d_bps;
                mssecleft = d_bps * 1000;
			...
			if (Verbose > 1) {
				...
				vstringf (_("Bytes Sent: %7ld/ %7ld   BPS:%-8ld ETA %02d:%02d.%03d  \n"),
					 (long) zi->bytes_sent, (long) zi->bytes_total, 
					last_bps, minleft, secleft,mssecleft);
               //
			}
